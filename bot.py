...
async def download_platform(
    ...
    try:
        info_proc = await asyncio.get_running_loop().run_in_executor(
            ...
            lambda: subprocess.run(
                ...
                + yt_extractor_args + playlist_flag + [url],
    ...
    ] + yt_extractor_args + playlist_flag + quality_flags + [url]
    ...
    try:
        ...
        if result.returncode != 0:
            logger.error(f"yt-dlp falló (código {result.returncode}): {result.stderr[:300]}")
            return [], title
    except Exception as e:
...
async def upload_to_cloud(
    ...
    identifier = re.sub(r"[^a-zA-Z0-9_-]", "-", f"tgbot-{sanitize_name(title)}-{uuid.uuid4().hex[:8]}")[:80]
    metadata = {
        "title": title,
        "mediatype": "data",
        ...
        for file_path in downloaded_files:
            ...
            try:
                archive_path = await compress_file_with_progress(
                    file_path, work_dir, archive_name, chat_id, smid, app
                )
            except Exception as e:
                await safe_edit(app, chat_id, smid,
...
def _validate_config() -> list[str]:
    errors = []
    if not BOT_TOKEN or not BOT_TOKEN.replace("-", "").replace(":", "").isalnum():
